...
"")
        ...
        self.registerTable.setObjectName("Table")
        ...
        self.listWidget.setGeometry(QtCore.QRect(1050, 110, 550, 500))
        font = QtGui.QFont()
        font.setFamily("Sitka Small")
        font.setPointSize(10)
        ...
        self.listWidget3.setGeometry(QtCore.QRect(1470, 630, 340, 300))
        font = QtGui.QFont()
        font.setFamily("Sitka Small")
        font.setPointSize(10)
        ...
        self.forwarding_box.setGeometry(QtCore.QRect(1650, 110, 150, 30))
        font = QtGui.QFont()
        font.setFamily("Sitka Small")
        font.setPointSize(10)
        ...
        self.register_box.setGeometry(QtCore.QRect(1650, 140, 150, 30))
        font = QtGui.QFont()
        font.setFamily("Sitka Small")
        font.setPointSize(10)
        ...
        self.inputButton.setGeometry(QtCore.QRect(1610, 190, 210, 30))
        font = QtGui.QFont()
        font.setFamily("Sitka Small")
        font.setPointSize(10)
        ...
        self.datacacheTable.setRowCount(pipelining.numSA)
        ...
        self.instcacheTable.setRowCount(pipelining.numSA)
...
#-----------------------------------------------------------------------------------------------
    ...
    def step_clicked(self): # Function for self button
        if pipelining.register_counter != -2: #Only do things if there are instructions left
            ...
            # For buffers or stats in listwidget 1
            if pipelining.register_counter == -2:
                pipelining.print_stmt()
                item = QListWidgetItem("Number of accesses in data cache: " + str(pipelining.accesses))
                self.cacheStatlistWidget.addItem(item)
                item = QListWidgetItem("Number of hits in data cache: " + str(pipelining.hits))
                self.cacheStatlistWidget.addItem(item)
                item = QListWidgetItem("Number of misses in data cache: " + str(pipelining.misses))
                self.cacheStatlistWidget.addItem(item)
                item = QListWidgetItem("Number of accesses in instruction cache: " + str(pipelining.I_accesses))
                self.cacheStatlistWidget.addItem(item)
                item = QListWidgetItem("Number of hits in instruction cache: " + str(pipelining.I_hits))
                self.cacheStatlistWidget.addItem(item)
                item = QListWidgetItem("Number of misses in instruction cache: " + str(pipelining.I_misses))
                self.cacheStatlistWidget.addItem(item)
                ...
                if (pipelining.print_cache["fetch"]!=None):
                    item = QListWidgetItem("Set accessed for fetch: " + str(pipelining.print_cache["fetch"]))
                    self.cacheStatlistWidget.addItem(item)
                if (pipelining.print_cache["load"]!=None):
                    item = QListWidgetItem("Set accessed for load: " + str(pipelining.print_cache["load"]))
                    self.cacheStatlistWidget.addItem(item)
                if (pipelining.print_cache["store"]!=None):
                    item = QListWidgetItem("Set accessed for store: " + str(pipelining.print_cache["store"]))
                    self.cacheStatlistWidget.addItem(item)
                if (pipelining.print_cache["f_access"]!=None):
                    item = QListWidgetItem("Fetch access: " + str(pipelining.print_cache["f_access"]))
                    self.cacheStatlistWidget.addItem(item)
                if (pipelining.print_cache["l_access"]!=None):
                    item = QListWidgetItem("Load access: " + str(pipelining.print_cache["l_access"]))
                    self.cacheStatlistWidget.addItem(item)
                if (pipelining.print_cache["s_access"]!=None):
                    item = QListWidgetItem("Store access: " + str(pipelining.print_cache["s_access"]))
                    self.cacheStatlistWidget.addItem(item)
                pipelining.reset_print_cache()
            ...
            for i in range(pipelining.numSA):
                temp = pipelining.data_cache[set_num][i]
                for j in range(3):
            ...
            for i in range(pipelining.numSA):
                temp = pipelining.instr_cache[set_num][i]
                for j in range(3):
    ...
    def take_main_input(self):
        ...
        if done1 and done2 and done3:
            pipelining.cache_size = int(cs)
            pipelining.block_size = int(bs)
            pipelining.numSA = int(nw)
            pipelining.intialise_cache()
    ...
    def dcblock_input(self):
        ...
        if done:
            ...
            set_num = self.data_cache_set - 1
            temp = pipelining.data_cache[set_num][block_num]
            for el in temp[-1]:
                item = QListWidgetItem(str(el))
                self.dclistWidget.addItem(item)
                print(el)
    ...
    def icblock_input(self):
        ...
        if done:
            ...
            set_num = self.inst_cache_set - 1
            temp = pipelining.instr_cache[set_num][block_num]
            for el in temp[-1]:
                item = QListWidgetItem(str(el))
                self.iclistWidget.addItem(item)
